...
function AddDatePickers() {
    ...
    var calendarVisible = window.DatePickerTable.style.display === 'table';
    var isDatePickerInput = target.classList.contains('date-picker');
    var isCalendarCell = target.tagName === 'TD' && target.parentNode.parentNode.parentNode.classList.contains('date-picker-table');

    // Check if the clicked target is inside the calendar or the date picker input
    var isInsideCalendar = isDatePickerInput || isCalendarCell;

    // Check if the clicked target is not inside the calendar or the date picker input
    var isOutsideCalendar = !isInsideCalendar;

    if (isOutsideCalendar && calendarVisible) {
        ClearDatePicker();
    }
    });

    window.addEventListener('scroll', function(event) {
        if (window.DatePickerTable.style.display === 'table') {
            ClearDatePicker();
        }
    });
}
...
function AddCalendarDays(Year, Month, Object) {
    ...
    var HeaderRow = "<table><tr><td onclick='event.stopPropagation(); AddCalendarDays(window.Year, window.Month - 1);'>❮</td><td onclick='AddCalendarMonths(window.Year);'>" + window.Months[window.Month] + " " + window.Year.toString() + "</td><td onclick='event.stopPropagation(); AddCalendarDays(window.Year, window.Month + 1);' >❯</td></tr></table>";
